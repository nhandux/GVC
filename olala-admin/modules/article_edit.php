	...
	// elseif(empty($content)) $error = '<span class="show-error">Vui lòng nhập nội dung chi tiết.</span>';
		...
		if($OK) {
			...
			$data = array(
				...
				'link_face'=>$db->clearText($link_face),
				'link_tw'=>$db->clearText($link_tw),
				'link_in'=>$db->clearText($link_in),
			...
			if($handleUploadImg) {
				...
				if($imgUp->processed) {
					...
					if($cate_id==4) {
					$imgUp->file_new_name_body = 'industri-' . $name_image;
					$imgUp->image_resize = true;
					$imgUp->image_ratio_crop = true;
					$imgUp->image_y = 634;
					$imgUp->image_x = 1600;
					...
					$imgUp->file_new_name_body = 'career-' . $name_image;
					$imgUp->Process($dir_dest);
				}
				else if($cate_id==8) {
					$imgUp->file_new_name_body = 'career-' . $name_image;
					...
					$imgUp->file_new_name_body = 'exp-' . $name_image;
					$imgUp->image_resize = true;
					$imgUp->image_ratio_crop = true;
					$imgUp->image_y = 393;
					$imgUp->image_x = 606;
					...
					$imgUp->file_new_name_body = 'ser-' . $name_image;
					$imgUp->image_resize = true;
					$imgUp->image_ratio_crop = true;
					$imgUp->image_y = 385;
					$imgUp->image_x = 670;
					...
					$imgUp->file_new_name_body = 'mem-' . $name_image;
					$imgUp->image_resize = true;
					$imgUp->image_ratio_crop = true;
					$imgUp->image_y = 309;
					$imgUp->image_x = 733;
					...
					$imgUp->file_new_name_body = 'team-' . $name_image;
					$imgUp->image_resize = true;
					$imgUp->image_ratio_crop = true;
					$imgUp->image_y = 240;
					$imgUp->image_x = 240;
...
if(!$OK) article("?".TTH_PATH."=article_edit", "edit", $article_id, $article_menu_id, $name, $title, $description, $keywords, $img, $img_note,$link_face, $link_tw, $link_in, $comment, $content, $is_active, $hot, $created_time, $upload_img_id, $error);
?>