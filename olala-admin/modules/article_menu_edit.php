	...
	if(empty($name)) $error = '<span class="show-error">Vui lòng nhập tên chuyên mục.</span>';
		...
		if($OK) {
			...
			if($handleUploadImg) {
				...
				if($category_id==1) {
					$imgUp->file_new_name_body = 'career-' . $img_name_file;
					$imgUp->Process($dir_dest);
				}else if($category_id==10) {
					$imgUp->file_new_name_body = 'focus-' . $img_name_file;
					$imgUp->Process($dir_dest);
				}else if($category_id==11) {
					$imgUp->file_new_name_body = 'about-' . $img_name_file;
					$imgUp->Process($dir_dest);
				}else if($category_id==12) {
					$imgUp->file_new_name_body = 'for-' . $img_name_file;
					$imgUp->Process($dir_dest);
				}else{

				$imgUp->file_new_name_body    = 'post-' . $img_name_file;
				$imgUp->image_resize          = true;
				$imgUp->image_ratio_crop      = true;
				$imgUp->image_y               = 280;
				$imgUp->image_x               = 380;
