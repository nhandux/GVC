...
function getDsAr($name){
	global $db;
	$db->table = "category";
	$db->condition = "name = '".$name."'";
	$db->order = "";
	$db->limit = "1";
	$rows = $db->select();
	$cate_id = $rows[0]['category_id'];
		// foreach($rows as $row){
		// 	$category_id = $row['category_id'];
		// }
		$db->table = "article_menu";
		$db->condition = "category_id = '".$cate_id."'";
		$db->order = "";
		$db->limit = "";
		$rows1 = $db->select();	
			if($db->RowCount>0) {
				foreach ($rows1 as $row) {
					$id_armenu = $row['article_menu_id'];
					$db->table = "article";
					$db->condition = "article_menu_id = '".$id_armenu."'";
...
function getSoPo($tin){
			global $db;
			$db->table = "article_menu";
            $db->condition = "`is_active` = 1 AND `category_id` = 12";
            $db->order = "";
            $db->limit = $tin.",1";
            $rows = $db->select();
            return $rows;
